...
export function AuthProvider({ children }) {
    ...
    useEffect(() => {
        try {
            const storedToken = localStorage.getItem("Cell Tech BD_token");
            const storedUser = localStorage.getItem("Cell Tech BD_user");
            ...
            localStorage.removeItem("Cell Tech BD_token");
            localStorage.removeItem("Cell Tech BD_user");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const persistAuth = useCallback((newToken, newUser) => {
        try {
            localStorage.setItem("Cell Tech BD_token", newToken);
            localStorage.setItem("Cell Tech BD_user", JSON.stringify(newUser));
        } catch (error) {
    ...
    const updateProfile = useCallback(async (profileData) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setUser(updatedUser);
                try {
                    localStorage.setItem("Cell Tech BD_user", JSON.stringify(updatedUser));
                } catch (e) {
